...
batch_size = 32
def pad_sequence(sequences,maxlen=None,dtype='int32',padding='post',truncating='post',value=0.0):
    ans = []
    for item in sequences:
        if len(item) > maxlen:
            ans.append(item[:maxlen])
        else:
            ans.append(item + [value]*(maxlen-len(item)))
    return ans
...
def getData4Bert(FNAME, tokenizer, sentenceLabel2idx=None, tag2idx=None, MAX_LEN=None):
    ...
    input_ids = pad_sequence([tokenizer.convert_tokens_to_ids(txt) for txt in tokenized_texts],
                              maxlen=MAX_LEN, value=0.0,
                              truncating="post", padding="post")

    tags = pad_sequence([[tag2idx.get(l) for l in lab] for lab in labels],
    ...
    return torch.tensor(input_ids).int(), torch.tensor(attention_masks), torch.tensor(tags), torch.tensor(sent_tags), sentenceLabel2idx, tag2idx, MAX_LEN
